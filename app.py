...
def enter_data(id, brand, address):
    if request.method == 'POST':
        ...
        # date_and_time = date_now.strftime("%d/%m/%Y %H:%M:%S")
        match busy_select:
            case "Buzzing Bliss":
                busy = 1
            case "Gentle Gliding":
        ...
        if id == 'M':
            ...
            if id_station == 'M':
                message = f"A Gas Station does not exist at address: {
                    location}"
                flash(message)
                return render_template('enter_data.html', id=id, brand=brand, address=address)

        if (not collection.find_one({"_id": id_station})):
            post = {"_id": id_station, "brand": name,
                    "address": location, "busy_status": [], "price": []}
            collection.insert_one(post)

        if busy:
            collection.update_one({"_id": id_station}, {
                                  "$push": {"busy_status": busy_list}})
        if price:
            collection.update_one({"_id": id_station}, {
                                  "$push": {"price": price_list}})
    return render_template('enter_data.html', id=id, brand=brand, address=address)
        ...
        if (stations_found):
            ...
            for price in station_price_list:
                ...
                if (difference < 900):
                    ...
                    cost_list.insert(
                        0, (round((difference/60), 0), price_entry))

        total_busy_score = round(
            total_busy_score/total_weight, 1) if total_weight > 0 else total_busy_score

        temp = (brand_name, address, cost, total_busy_score,
                brand_id, busy_list, cost_list)
